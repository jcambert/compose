...
The future GUI must reuse the same application services, command descriptors and command intent models as the CLI. The CLI must not contain business rules that a GUI would need to duplicate.
...
- Add GUI components that render forms from guided descriptors.
